...
export default function DashboardLeaderboard() {
    ...
    const getMedalColor = (index: number) => {
        switch (index) {
            case 0:
                return 'text-blue-400 drop-shadow-[0_0_15px_rgba(96,165,250,0.6)] animate-pulse';
            case 1:
                return 'text-blue-500/80 drop-shadow-[0_0_10px_rgba(59,130,246,0.4)]';
            case 2:
                return 'text-blue-600/60 drop-shadow-[0_0_8px_rgba(37,99,235,0.3)]';
            default:
    ...
    const LeaderboardSkeleton = () => (
        <div className="space-y-4 animate-pulse">
            {[1, 2, 3, 4, 5].map((i) => (
                <div
                    key={i}
                    className="h-20 bg-card/5 rounded-2xl border border-foreground/5 w-full"
                />
            ))}
        </div>
    );
                                        ...
                                        >
                                            <div className="flex items-center gap-3 sm:gap-4 md:gap-6 min-w-0">
                                                ...
                                                <Avatar className="w-10 h-10 sm:w-12 sm:h-12 border border-foreground/10 shrink-0">
                                                    ...
                                                    />
                                                    <AvatarFallback className="bg-primary/30 text-primary font-bold text-sm sm:text-base">
                                                        {
                                                            (user.firstName ||
                                                                'U')[0]
                                                        }
                                        ...
                                        >
                                            <div className="flex items-center gap-3 sm:gap-4 md:gap-6 min-w-0">
                                                ...
                                                <Avatar className="w-10 h-10 sm:w-12 sm:h-12 border border-foreground/10 shrink-0">
                                                    ...
                                                    />
                                                    <AvatarFallback className="bg-orange-500/30 text-orange-500 font-bold text-sm sm:text-base">
                                                        {
                                                            (user.firstName ||
                                                                'U')[0]
                                                        }
                                                ...
                                                <div className="min-w-0 overflow-hidden">
                                                    ...
                                                    <p className="text-[10px] sm:text-xs opacity-60 font-medium truncate">
                                                        {user.pet
                                                            ? `${user.pet.name} (Lvl ${user.pet.level})`
                                                            : 'Scholar'}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Flame
                                                        size={12}
                                                        className="text-orange-500 fill-orange-500 sm:w-3.5 sm:h-3.5"
                                                    />
                                                    <span className="font-black text-base sm:text-xl tracking-tight">
                                                        {user.streak}
                                                    </span>
                ...
                <div className="podium-card order-2 md:order-1 flex flex-col items-center w-full md:w-1/3 max-w-[200px] sm:max-w-[240px]">
                    <div className="relative mb-3 sm:mb-4">
                        <Avatar className="w-16 h-16 sm:w-20 sm:h-20 border-2 sm:border-4 border-gray-300 shadow-[0_0_15px_rgba(209,213,219,0.3)]">
                            <AvatarImage
                                src={
                                    second.profilePicturePath ||
                                    `https://api.dicebear.com/7.x/notionists/svg?seed=${second.email}`
                                }
                            />
                            <AvatarFallback className="bg-gray-300 text-gray-900 font-bold text-lg sm:text-xl">
                                {(second.firstName || 'U')[0]}
                            </AvatarFallback>
                        </Avatar>
            ...
            <div className="podium-card order-1 md:order-2 flex flex-col items-center w-full md:w-1/3 max-w-[240px] sm:max-w-[280px] mb-4 md:mb-0 z-10">
                <div className="relative mb-4 sm:mb-6">
                    ...
                    <Avatar className="w-24 h-24 sm:w-28 sm:h-28 border-2 sm:border-4 border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.4)]">
                        <AvatarImage
                            src={
                                first.profilePicturePath ||
                                `https://api.dicebear.com/7.x/notionists/svg?seed=${first.email}`
                            }
                        />
                ...
                <div className="text-center p-6 sm:p-8 bg-yellow-400/10 border border-yellow-400/30 rounded-[1.5rem] sm:rounded-[2rem] w-full backdrop-blur-xl relative overflow-hidden shadow-[0_0_40px_rgba(250,204,21,0.1)] group hover:scale-[1.02] transition-transform duration-300">
                    ...
                    <p className="text-[10px] sm:text-xs text-foreground/60 dark:text-yellow-500/80 truncate mb-3 sm:mb-4 font-bold tracking-wide uppercase">
                        {(first.institution || 'Izabi Champion').substring(
                            0,
                            20,
                        )}
                    </p>
                ...
                <div className="podium-card order-3 md:order-3 flex flex-col items-center w-full md:w-1/3 max-w-[200px] sm:max-w-[240px]">
                    <div className="relative mb-3 sm:mb-4">
                        <Avatar className="w-16 h-16 sm:w-20 sm:h-20 border-2 sm:border-4 border-amber-600 shadow-[0_0_15px_rgba(217,119,6,0.3)]">
                            <AvatarImage
                                src={
                                    third.profilePicturePath ||
                                    `https://api.dicebear.com/7.x/notionists/svg?seed=${third.email}`
                                }
                            />
