            ...
            <div className="flex flex-col md:flex-row items-end justify-center gap-4 md:gap-8 mb-12 min-h-[300px] px-4">
                ...
                <div className="podium-card order-1 md:order-2 flex flex-col items-center w-full md:w-1/3 max-w-[280px] -mt-12 md:-mt-0 z-10">
                    ...
                    <div className="text-center p-8 bg-gradient-to-b from-yellow-400/10 to-transparent border border-yellow-400/30 rounded-[2rem] w-full backdrop-blur-xl relative overflow-hidden shadow-[0_0_50px_rgba(250,204,21,0.1)] group hover:scale-[1.02] transition-transform duration-300">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40 mt-2">{type === 'xp' ? 'Experience Points' : 'Consecutive Days'}</p>
                    </div>
                </div>
    ...
    return (
        <div ref={containerRef} className="space-y-8 pb-20 w-full max-w-6xl mx-auto px-4 md:px-0">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
                ...
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md flex items-center gap-4 min-w-[200px]">
            ...
            <Tabs defaultValue="xp" className="w-full" onValueChange={setActiveTab}>
                <div className="flex justify-center mb-10">
                    <TabsList className="bg-white/5 border border-white/10 p-1.5 rounded-full h-14">
                        <TabsTrigger value="xp" className="rounded-full px-8 h-full font-bold uppercase text-xs tracking-wider data-[state=active]:bg-primary data-[state=active]:text-white transition-all gap-2">
                            <Zap size={16} /> Total XP
                        </TabsTrigger>
                        <TabsTrigger value="streak" className="rounded-full px-8 h-full font-bold uppercase text-xs tracking-wider data-[state=active]:bg-orange-500 data-[state=active]:text-white transition-all gap-2">
                            <Flame size={16} /> Top Streaks
                        </TabsTrigger>
                    </TabsList>
                </div>

                <TabsContent value="xp" className="space-y-10">
                    <Podium users={leaderboardData.topStudents || []} type="xp" />
                            ...
                            <div className="space-y-2">
                                {(leaderboardData.topStudents || []).slice(3).map((user, i) => (
                                    <div 
                                        key={user._id} 
                                        className={cn(
                                            "leaderboard-item flex items-center justify-between p-4 rounded-2xl border border-transparent transition-all hover:bg-white/5",
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <span className="font-black text-xl tracking-tight">{user.points.toLocaleString()}</span>
                                            <p className="text-[10px] font-bold uppercase tracking-widest opacity-30">XP</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <TabsContent value="streak" className="space-y-10">
                    <Podium users={leaderboardData.topStreaks || []} type="streak" />
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <span className="font-black text-xl tracking-tight">{user.streak}</span>
                                            </div>
                                            <p className="text-[10px] font-bold uppercase tracking-widest opacity-30">Days</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
