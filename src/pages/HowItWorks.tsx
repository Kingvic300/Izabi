        ...
        >
                ...
                <div className="w-full px-6 lg:px-12">
                    <div className="space-y-32">
                        {steps.map((step, i) => (
                            <div
                                key={i}
                                className={`step-card flex flex-col ${i % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-20`}
                            >
                                <div className="flex-1 space-y-8">
                                    <div
                                        className={`w-20 h-20 rounded-2xl ${step.bg} ${step.color} flex items-center justify-center shadow-2xl group`}
                                    >
                                        {step.icon}
                                    </div>
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4">
                                            <span className="text-5xl font-bold opacity-10">
                                                {i + 1}
                                            </span>
                                    ...
                                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {step.details.map((detail, j) => (
                                            <li
                                                key={j}
                                                className="flex items-center gap-3 glass p-4 rounded-xl border border-foreground/5"
                                            >
                                                <CheckCircle
                                                    size={18}
            ...
            <section className="py-44 relative z-10">
                <div className="w-full px-6 lg:px-12 text-center bg-gradient-hero/5 py-24 rounded-[64px] border border-foreground/5 relative overflow-hidden">
                    <h2 className="text-5xl font-bold mb-8 leading-tight text-foreground">
                        {t('hiw.cta_title')}
                    </h2>
                    <Link to="/signup">
                        <Button
