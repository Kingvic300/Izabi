...
export const PDFSplitterModal: React.FC<PDFSplitterModalProps> = ({
    ...
    const chapterSuggestions = suggestions.filter(
        (s) => s.strategy === 'chapter',
    );
    const rangeSuggestions = suggestions.filter(
        (s) => s.strategy === 'page-range',
    );
    ...
    const handleSelectAll = (strategyType: 'chapter' | 'page-range') => {
        const stratSuggestions =
            strategyType === 'chapter' ? chapterSuggestions : rangeSuggestions;
        const newSelected = new Set(selectedSuggestions);

        const allSelected = stratSuggestions.every((s) =>
        ...
        try {
            ...
            if (selected.length === 1) {
                await onProcessSection({
                    ...
                    sectionTitle:
                        selected[0].detectedTitle || selected[0].label,
                });
            } else {
                await onProcessAll(
                    selected.map((s) => ({
                    ...
                    <CardContent className="p-6 space-y-6">
                        ...
                        <div className="flex gap-3">
                            {chapterSuggestions.length > 0 && (
                                <Button
                                    variant={
                                        selectedStrategy === 'chapter'
                                            ? 'default'
                                            : 'outline'
                                    }
                                    onClick={() =>
                                        setSelectedStrategy('chapter')
                                    }
                            ...
                            <Button
                                variant={
                                    selectedStrategy === 'page-range'
                                        ? 'default'
                                        : 'outline'
                                }
                                onClick={() =>
                                    setSelectedStrategy('page-range')
                                }
                                className="flex-1 h-14 rounded-xl font-bold"
                            >
                                    ...
                                    <div className="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto pr-2">
                                        {(selectedStrategy === 'chapter'
                                            ...
                                            <SuggestionCard
                                                ...
                                                selected={selectedSuggestions.has(
                                                    suggestion.id,
                                                )}
                                                onToggle={() =>
                                ...
                                >
                                    <div className="p-6 rounded-2xl bg-card/5 border border-foreground/10">
                                        ...
                                        <p className="text-sm text-muted-foreground mt-4">
                                            Processing pages {customStart} to{' '}
                                            {customEnd} (
                                            {customEnd - customStart + 1} pages)
                                        </p>
                                    </div>
                                </motion.div>
                            )}
