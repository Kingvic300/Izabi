        ...
        >
            <div className="flex items-center p-4 gap-4">
                ...
                <div className="relative h-14 w-14 flex-shrink-0">
                    <svg className="h-full w-full -rotate-90">
                        ...
                        <motion.circle
                            ...
                            animate={{ strokeDashoffset: 150.8 - (150.8 * job.progress) / 100 }}
                            className={cn(
                                "transition-all duration-500 ease-out",
                                isCompleted ? "text-green-500" : isFailed ? "text-red-500" : "text-primary shadow-[0_0_10px_theme(colors.primary.DEFAULT)]"
                            )}
                        />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                        {isCompleted ? (
                            ...
                            <span className="text-[10px] font-black font-mono text-foreground/80">{Math.round(job.progress)}%</span>
                        )}
                    </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-foreground truncate">{getTitle()}</h4>
                    <p className="text-xs text-muted-foreground truncate font-medium">
                        {isProcessing && <span className="inline-block w-2 h-2 rounded-full bg-primary animate-pulse mr-2" />}
                        {getStatusMessage()}
                    </p>
                </div>
                ...
                >
                    {isCompleted || isFailed ? (
                        <span className="text-[10px] font-bold uppercase tracking-wider px-2">Clear</span>
                    ) : (
                        <span className="text-[10px] font-bold uppercase tracking-wider px-2">Hide</span>
                    )}
                </button>
            </div>
