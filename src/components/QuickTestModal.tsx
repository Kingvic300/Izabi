...
import { X, Clock, Zap, CheckCircle, XCircle, Trophy, Loader2, AlertCircle } from 'lucide-react';
...
const QuickTestModal: React.FC<QuickTestModalProps> = ({ isOpen, onClose, onComplete }) => {
    ...
    const startTest = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Failed to start test. Upload study materials first!');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-xl border-b border-foreground/10 p-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-2xl font-bold">{testData?.title || 'Quick Test'}</h2>
                                    <p className="text-sm text-muted-foreground">
                                        {results ? 'Test Complete' : 'Answer all questions before time runs out'}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                    ...
                    <div className="p-6 space-y-6">
                        {loading && (
                            <div className="flex flex-col items-center justify-center py-20 space-y-4">
                                ...
                                <p className="text-lg font-medium text-muted-foreground">Generating your personalized test...</p>
                            </div>
                        )}
                        ...
                        {results && (
                            <div className="space-y-6">
                                ...
                                <div className="p-8 rounded-3xl bg-gradient-to-br from-primary/20 to-accent/10 border border-primary/30 text-center">
                                    <Trophy size={64} className="mx-auto mb-4 text-primary" />
                                    <h3 className="text-4xl font-bold mb-2">{results.score}%</h3>
                                    ...
                                    {results.score >= 70 && (
                                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/20 text-primary font-bold">
                                            <Zap size={16} />
                                            +{results.pointsEarned || 0} XP Earned
                                        </div>
                                    )}
                                </div>
                                        ...
                                        >
                                            <div className="flex items-start gap-3 mb-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="font-bold mb-2">{result.text}</p>
                                                    <p className="text-sm">
                                ...
                                <Button onClick={handleClose} className="w-full h-14 rounded-2xl text-lg font-bold">
                                    Close
                                </Button>
                            </div>
                        )}
