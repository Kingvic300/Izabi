...
const MONGODB_URI =
    ...
    'mongodb+srv://kingvic300:victor123@cluster0.ot4ik.mongodb.net/izabi_db?retryWrites=true&w=majority';
...
async function createAdmin() {
    ...
    try {
        ...
        const firstName =
            (await question('First Name (optional, press Enter to skip): ')) ||
            'Admin';
        const lastName =
            (await question('Last Name (optional, press Enter to skip): ')) ||
            'User';
        ...
        if (existingUser) {
            ...
            console.log(
                `   Found: ${existingUser.email} (Role: ${existingUser.role || 'USER'})`,
            );
            process.exit(1);
        }
